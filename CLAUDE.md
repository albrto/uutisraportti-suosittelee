...
Consequences:
- Pipeline/validator fixes to `pipeline/*.py` must also be ported to the `scripts/` copies (which is what Actions runs) and pushed. The copies live side by side in this repo now — diff them when in doubt. (Unifying them into single dual-use scripts is a planned but not yet executed improvement.)
- Apply corrections by pushing `admin/korjaukset.json` on main (triggers the workflow). **Do not run `pipeline/paivita_korjaukset.sh` → `julkaise.sh` without first reconciling `pipeline/suositukset.json` with the repo root's** — it would publish the stale local copy and revert months of episodes.
...
RSS (feeds.captivate.fm)
    │
    ▼
pipeline/uutisraportti_automaatio_deepgram_claude.py
    │  ─ downloads MP3
...
pipeline/validoi_suosittelijat.py   ← the ONLY local validation-data generator since 16.9.2026
    ...
    ▼
pipeline/laheta_ilmoitus.py → Netlify Forms POST (form-name: "automaatio-ilmoitus")
```
...
3. `pipeline/sovella_korjaukset.py` rewrites `suosittelija` fields in-place inside `pipeline/suositukset.json` (with timestamped backup), then `paivita_korjaukset.sh` regenerates validation data and runs `../julkaise.sh` (copies `pipeline/suositukset.json` → repo root, commits, pushes → Netlify).
...
**⚠️ Correction schema divergence (as of Aug 2026):** `admin/korjaukset.json` uses a **new full-record schema** `{jakso_id, r_idx, teos, uusi_data: {teos, suosittelija, kuvaus, paakategoria, kategoriat}}` — it can correct any field (including `teos`), and `scripts/` applies `uusi_data` wholesale. The **pipeline/** scripts still expect the old rename-only schema `{jakso_id, r_idx, vanha_suosittelija, uusi_suosittelija}`: `sovella_korjaukset.py` skips entries whose current value doesn't match `vanha_suosittelija`, so new-schema entries merged in by `synkronoi_pilvi_tiedostot.py` are inert locally (harmless — they're already applied on the production side). New corrections should be pushed as `admin/korjaukset.json` on main (new schema), which triggers the production workflow.
